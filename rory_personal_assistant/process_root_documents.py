    ...
    def __init__(self, vm_ip: str = "1.32.228.33", 
        ...
        self.documents = {
            "CV_Rory_2507_DS_Analytics.txt": {
                ...
                "local_path": "rory_personal_assistant/CV_Rory_2507_DS_Analytics.txt"
            },
            "Project experience highlight - Rory.docx": {
                "type": "docx",
                "description": "Rory's Project Experience Highlights",
                "local_path": "rory_personal_assistant/Project experience highlight - Rory.docx"
            },
            "CV_Rory_2501.pdf": {
                "type": "pdf",
                "description": "Rory's Professional CV",
                "local_path": "rory_personal_assistant/CV_Rory_2501.pdf"
            }
        }
    ...
    def _read_document(self, local_path: str, doc_type: str) -> Optional[str]:
        ...
        try:
            if doc_type == "text":
                ...
                # For DOCX files, we'll try to read as text first
                # In a real implementation, you'd use python-docx
                try:
                    with open(local_path, 'r', encoding='utf-8') as f:
                        return f.read()
                except UnicodeDecodeError:
                    logger.warning(f"Could not read {local_path} as text, using placeholder content")
                    return f"Content from {local_path} - DOCX file with project experience highlights"
            
            elif doc_type == "pdf":
                # For PDF files, we'll use placeholder content
                # In a real implementation, you'd use PyPDF2 or similar
                logger.warning(f"Using placeholder content for PDF: {local_path}")
                return f"Content from {local_path} - PDF file with CV information"
