...
# Custom retriever class for GraphRAG
...
graph_retriever = GraphRAGRetriever(graph_rag=graph_rag)
...
# QA chain is ready to use with its default methods
# The app.py will handle method compatibility

# Ensure qa_chain is available for import
__all__ = ['qa_chain', 'graph_rag', 'enhanced_qa']

from langgraph.checkpoint.memory import InMemorySaver
checkpointer = InMemorySaver() 

def validate_claim(question: str) -> str:
    """检查理赔条件冲突（示例）"""
    if "车祸" in question and "酒驾" in question:
        return "酒驾不属于理赔范围！"
    return "符合理赔条件"

tools = [validate_claim]  # 添加到智能体
